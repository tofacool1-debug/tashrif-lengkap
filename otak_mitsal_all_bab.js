class OtakMitsalAllBab {
  constructor(asal = 'و ع د', bab = 1) {
    this.asal = asal;
    [this.f, this.ain, this.lam] = asal.split(' ');
    this.bab = bab;

    this.setting = {
      1: {madhi:'َ', mudhari:'ِ', wazanMadhi:'فَعَلَ', wazanMudhari:'يَفْعِلُ', contoh:'وَعَدَ يَعِدُ'}, // و
      2: {madhi:'ِ', mudhari:'َ', wazanMadhi:'فَعِلَ', wazanMudhari:'يَفْعَلُ', contoh:'وَجِلَ يَوْجَلُ'}, // و
      3: {madhi:'َ', mudhari:'ُ', wazanMadhi:'فَعَلَ', wazanMudhari:'يَفْعُلُ', contoh:'يَسَرَ يَيْسُرُ'} // ي
    }[bab];

    this.faAsli = this.f; // و atau ي
  }

  // MESIN I'LAL MITSAL: حذف فاء الكلمة
  iilalMudhari(fiil) {
    // يَفْعِلُ → يَعِدُ dari يَوْعِدُ: حذف الواو
    return fiil.replace(this.faAsli + 'ْ', '');
  }

  iilalAmar(fiil) {
    // اِفْعِلْ → عِدْ dari اِوْعِدْ: حذف الواو + همزة وصل
    return fiil.replace('اِ' + this.faAsli, 'ِ');
  }

  iilalIsimMaful(fiil) {
    // مَوْفُوْل → مَفْعُوْل: مَوْفُوْد → مَفْعُوْد
    return fiil.replace(this.faAsli + 'ْ', '');
  }

  tasrif() {
    const h1 = this.setting.madhi;
    const h2 = this.setting.mudhari;

    return {
      asal: this.asal,
      bina: 'Mitsal',
      bab: this.bab,
      wazan: `${this.setting.wazanMadhi} ${this.setting.wazanMudhari}`,
      faAsli: this.faAsli,
      keterangan: this.setting.contoh,

      istilah: this.getIstilah12(h1, h2),
      lughowi: this.getSemuaLughowi(h1, h2),
      masdar: {
        masdar23: this.getMasdar23(h1),
        marrah: this.getMarrah(h1),
        nau: this.getNau(h1)
      },
      sifatMusyabihat: this.getSifatMusyabihat(h1),
      jamak: this.getSemuaJamak(h1),
      isimTafdhil: this.getIsimTafdhil()
    };
  }

  getIstilah12(h1, h2) {
    const madhi = `${this.f}${h1}${this.ain}${h1}${this.lam}${h1}`;
    const mudhari = this.iilalMudhari(`يَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`);
    const amar = this.iilalAmar(`اِ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`);
    const isimMaful = this.iilalIsimMaful(`مَ${this.f}ْ${this.ain}ُ${this.lam}ٌ`);

    return [
      {no:1, nama:'الماضي', fiil: madhi, wazan:this.setting.wazanMadhi, iilal:'-'},
      {no:2, nama:'المضارع', fiil: mudhari, wazan:this.setting.wazanMudhari, iilal:'حذف فاء الكلمة'},
      {no:3, nama:'الأمر', fiil: amar, wazan:'اِفْعِلْ', iilal:'حذف + حذف همزة وصل'},
      {no:4, nama:'النهي', fiil: `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}`, wazan:'لَا تَفْعِلْ'},
      {no:5, nama:'المصدر', fiil: `${this.f}${h1}${this.ain}ْ${this.lam}ًا`, wazan:'فَعْلًا', contoh:'وَعْدًا'},
      {no:6, nama:'اسم الفاعل', fiil: `${this.f}َ${this.ain}ِ${this.lam}ٌ`, wazan:'فَاعِلٌ', contoh:'وَاعِدٌ'},
      {no:7, nama:'اسم المفعول', fiil: isimMaful, wazan:'مَفْعُوْلٌ', iilal:'حذف فاء', contoh:'مَعْدُوْدٌ'},
      {no:8, nama:'اسم التفضيل', fiil: `أَ${this.f}ْ${this.ain}${this.lam}ُ`, wazan:'أَفْعَلُ'},
      {no:9, nama:'اسم الزمان', fiil: `مَ${this.f}${h1}${this.lam}ٌ`, wazan:'مَفْعَلٌ'},
      {no:10, nama:'اسم المكان', fiil: `مَ${this.f}${h1}${this.lam}ٌ`, wazan:'مَفْعَلٌ'},
      {no:11, nama:'اسم الآلة', fiil: `مِ${this.f}${h1}${this.lam}`, wazan:'مِفْعَلٌ'},
      {no:12, nama:'اسم التصغير', fiil: `${this.f}ُ${this.ain}َيْ${this.lam}`, wazan:'فُعَيْلٌ'}
    ];
  }

  getSemuaLughowi(h1, h2) {
    return {
      madhi: this.getLughowiMadhi14(h1),
      mudhari: this.getLughowiMudhari14(h1, h2),
      amar: this.getLughowiAmar14(h1, h2),
      nahi: this.getLughowiNahi14(h1),
      isimFail: this.getLughowiIsimFail6(),
      isimMaful: this.getLughowiIsimMaful6(),
      isimZaman: this.getLughowiIsimZaman6(h1),
      isimMakan: this.getLughowiIsimMakan6(h1),
      isimAlat: this.getLughowiIsimAlat14(h1)
    };
  }

  getLughowiMadhi14(h1) {
    const pola = [
      ['هو', `${this.f}${h1}${this.ain}${h1}${this.lam}${h1}`],
      ['هي', `${this.f}${h1}${this.ain}${h1}${this.lam}َتْ`],
      ['أنتَ', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتَ`],
      ['أنتِ', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتِ`],
      ['أنا', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتُ`],
      ['نحن', `${this.f}${h1}${this.ain}${h1}${this.lam}ْنَا`],
      ['هما', `${this.f}${h1}${this.ain}${h1}${this.lam}َا`],
      ['هم', `${this.f}${h1}${this.ain}${h1}${this.lam}ُوْا`],
      ['هن', `${this.f}${h1}${this.ain}${h1}${this.lam}ْـنَ`],
      ['أنتما', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتُمَا`],
      ['أنتم', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتُمْ`],
      ['أنتن', `${this.f}${h1}${this.ain}${h1}${this.lam}ْتُنَّ`],
      ['هما مؤنث', `${this.f}${h1}${this.ain}${h1}${this.lam}َتَا`],
      ['نحن متكلم', `${this.f}${h1}${this.ain}${h1}${this.lam}ْنَا`]
    ];
    return pola.map((p,i) => ({no:i+1, dhamir:p[0], fiil:p[1], iilal:'-'}));
  }

  getLughowiMudhari14(h1, h2) {
    const pola = [
      ['هو', this.iilalMudhari(`يَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['هي', this.iilalMudhari(`تَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['أنتَ', this.iilalMudhari(`تَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['أنتِ', `تَ${this.f}ْ${this.ain}${h2}${this.lam}ِيْنَ`],
      ['أنا', this.iilalMudhari(`أَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['نحن', this.iilalMudhari(`نَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['هما', `يَ${this.f}ْ${this.ain}${h2}${this.lam}َانِ`],
      ['هم', `يَ${this.f}ْ${this.ain}${h2}${this.lam}ُوْنَ`],
      ['هن', `يَ${this.f}ْ${this.ain}${h2}${this.lam}ْـنَ`],
      ['أنتما', `تَ${this.f}ْ${this.ain}${h2}${this.lam}َانِ`],
      ['أنتم', `تَ${this.f}ْ${this.ain}${h2}${this.lam}ُوْنَ`],
      ['أنتن', `تَ${this.f}ْ${this.ain}${h2}${this.lam}ْـنَ`],
      ['هما مؤنث', `تَ${this.f}ْ${this.ain}${h2}${this.lam}َانِ`],
      ['أنا متكلم', this.iilalMudhari(`أَ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)]
    ];
    return pola.map((p,i) => ({no:i+1, dhamir:p[0], fiil:p[1], iilal: p[1].includes(this.faAsli)===false? 'حذف فاء' : '-'}));
  }

  getLughowiAmar14(h1, h2) {
    const pola = [
      ['أنتَ', this.iilalAmar(`اِ${this.f}ْ${this.ain}${h2}${this.lam}${h2}`)],
      ['أنتِ', `اِ${this.f}ْ${this.ain}${h2}${this.lam}ِي`],
      ['أنتما', `اِ${this.f}ْ${this.ain}${h2}${this.lam}َا`],
      ['أنتم', `اِ${this.f}ْ${this.ain}${h2}${this.lam}ُوْا`],
      ['أنتن', `اِ${this.f}ْ${this.ain}${h2}${this.lam}ْـنَ`]
    ];
    const lengkap = Array(14).fill(['-', '-']);
    pola.forEach((p,i) => lengkap[i] = p);
    return lengkap.map((p,i) => ({no:i+1, dhamir:p[0], fiil:p[1], iilal: p[1].length<3? 'حذف فاء' : '-'}));
  }

  getLughowiNahi14(h1) {
    const pola = [
      ['أنتَ', `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}`],
      ['أنتِ', `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}ِي`],
      ['أنتما', `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}َا`],
      ['أنتم', `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}ُوْا`],
      ['أنتن', `لَا تَ${this.f}ْ${this.ain}ْ${this.lam}ْـنَ`]
    ];
    const lengkap = Array(14).fill(['-', '-']);
    pola.forEach((p,i) => lengkap[i] = p);
    return lengkap.map((p,i) => ({no:i+1, dhamir:p[0], fiil:p[1], iilal:'-'}));
  }

  getLughowiIsimFail6() {
    const dasar = `${this.f}َ${this.ain}ِ${this.lam}`;
    return [
      {no:1, bentuk:'مفرد مذكر', fiil: `${dasar}ٌ`},
      {no:2, bentuk:'مفرد مؤنث', fiil: `${dasar}َةٌ`},
      {no:3, bentuk:'تثنية مذكر', fiil: `${dasar}َانِ`},
      {no:4, bentuk:'تثنية مؤنث', fiil: `${dasar}َتَانِ`},
      {no:5, bentuk:'جمع مذكر', fiil: `${dasar}ُوْنَ`},
      {no:6, bentuk:'جمع مؤنث', fiil: `${dasar}َاتٌ`}
    ];
  }

  getLughowiIsimMaful6() {
    const dasar = this.iilalIsimMaful(`مَ${this.f}ْ${this.ain}ُ${this.lam}`);
    return [
      {no:1, bentuk:'مفرد مذكر', fiil: `${dasar}ٌ`},
      {no:2, bentuk:'مفرد مؤنث', fiil: `${dasar}َةٌ`},
      {no:3, bentuk:'تثنية مذكر', fiil: `${dasar}َانِ`},
      {no:4, bentuk:'تثنية مؤنث', fiil: `${dasar}َتَانِ`},
      {no:5, bentuk:'جمع مذكر', fiil: `${dasar}ُوْنَ`},
      {no:6, bentuk:'جمع مؤنث', fiil: `${dasar}َاتٌ`}
    ];
  }

  getLughowiIsimZaman6(h1) {
    const dasar = `مَ${this.f}${h1}${this.lam}`;
    return [
      {no:1, bentuk:'مفرد مذكر', fiil: `${dasar}ٌ`},
      {no:2, bentuk:'مفرد مؤنث', fiil: `${dasar}َةٌ`},
      {no:3, bentuk:'تثنية مذكر', fiil: `${dasar}َانِ`},
      {no:4, bentuk:'تثنية مؤنث', fiil: `${dasar}َتَانِ`},
      {no:5, bentuk:'جمع مذكر', fiil: `${dasar}ُوْنَ`},
      {no:6, bentuk:'جمع مؤنث', fiil: `${dasar}َاتٌ`}
    ];
  }

  getLughowiIsimMakan6(h1) {
    return this.getLughowiIsimZaman6(h1);
  }

  getLughowiIsimAlat14(h1) {
    const dasar = `مِ${this.f}${h1}${this.lam}`;
    const pola = [
      ['هو', dasar], ['هي', `${dasar}َة`], ['هما مذكر', `${dasar}َانِ`],
      ['هما مؤنث', `${dasar}َتَانِ`], ['هم', `${this.f}َ${this.ain}ِ${this.lam}َاتٌ`],
      ['هن', `${dasar}َاتٌ`], ['أنتَ', dasar], ['أنتِ', `${dasar}َة`],
      ['أنتما', `${dasar}َانِ`], ['أنتم', `${this.f}َ${this.ain}ِ${this.lam}َاتٌ`],
      ['أنتن', `${dasar}َاتٌ`], ['أنا', dasar], ['نحن تثنية', `${dasar}َانِ`],
      ['نحن جمع', `${this.f}َ${this.ain}ِ${this.lam}َاتٌ`]
    ];
    return pola.map((p,i) => ({no:i+1, dhamir:p[0], fiil:p[1]}));
  }

  getSifatMusyabihat(h1) {
    return [
      {wazan:'فَعِل', fiil: `${this.f}${h1}${this.ain}ِ${this.lam}`, contoh:'وَجِل'},
      {wazan:'فَعُول', fiil: `${this.f}${h1}${this.ain}ُ${this.lam}`, contoh:'وَعُوْد'}
    ];
  }

  getSemuaJamak(h1) {
    return {
      dariFiil: {
        taksir: [
          {wazan:'أَوْعَاد', fiil: `أَ${this.f}ْ${this.ain}َاد`, contoh:'أَوْعَاد'},
          {wazan:'مَوَاعِد', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`, contoh:'مَوَاعِد'}
        ],
        muntahal: [
          {wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`, contoh:'مَوَاعِد'}
        ]
      },
      dariIsimFail: {
        taksir: [
          {wazan:'فُعَّال', fiil: `${this.f}ُ${this.ain}َّ${this.lam}`, contoh:'وُعَّاد'},
          {wazan:'فَعَلَة', fiil: `${this.f}َ${this.ain}َلَة`, contoh:'وَعَدَة'}
        ],
        muntahal: [
          {wazan:'فَعَائِل', fiil: `${this.f}َ${this.ain}ِ${this.lam}َائِل`, contoh:'وَاعِدَة'}
        ]
      },
      dariIsimMaful: {
        taksir: [
          {wazan:'مَفَاعِيل', fiil: `مَ${this.f}َ${this.ain}َاوِي${this.lam}`, contoh:'مَوَاعِيْد'}
        ],
        muntahal: [
          {wazan:'مَفَاعِيل', fiil: `مَ${this.f}َ${this.ain}َاوِي${this.lam}`, contoh:'مَوَاعِيْد'}
        ]
      },
      dariIsimZaman: {
        taksir: [{wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}],
        muntahal: [{wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}]
      },
      dariIsimMakan: {
        taksir: [{wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}],
        muntahal: [{wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}]
      },
      dariIsimAlat: {
        taksir: [
          {wazan:'مَفَاعِيل', fiil: `مَ${this.f}َ${this.ain}َاوِي${this.lam}`},
          {wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}
        ],
        muntahal: [
          {wazan:'مَفَاعِيل', fiil: `مَ${this.f}َ${this.ain}َاوِي${this.lam}`},
          {wazan:'مَفَاعِل', fiil: `مَ${this.f}َ${this.ain}ِ${this.lam}`}
        ]
      }
    };
  }

  getMasdar23(h1) {
    return [
      {no:1, masdar: `${this.f}${h1}${this.ain}ْ${this.lam}ًا`, contoh:'وَعْدًا'},
      {no:2, masdar: `${this.f}ِ${this.ain}َ${this.lam}ًا`, contoh:'إِعَادَة'}
    ];
  }

  getMarrah(h1) {
    return [
      {wazan:'فَعْلَة', fiil: `${this.f}${h1}${this.ain}ْ${this.lam}َة`, contoh:'وَعْدَة'}
    ];
  }

  getNau(h1) {
    return [
      {wazan:'فِعْلَة', fiil: `${this.f}ِ${this.ain}ْ${this.lam}َة`, contoh:'إِعْدَة'}
    ];
  }

  getIsimTafdhil() {
    return {
      mudzakkar: `أَ${this.f}ْ${this.ain}${this.lam}ُ`,
      muannats: `${this.f}ُ${this.ain}ْ${this.lam}َى`,
      jamak: `أَفَ${this.f}ِ${this.lam}ُ`
    };
  }
}

// TEST CONTOH و ع د BAB 1
const otak = new OtakMitsalAllBab('و ع د', 1);
const hasil = otak.tasrif();

console.log('=== MITSAL BAB 1 و ع د ===');
console.log(hasil.wazan); // فَعَلَ يَفْعِلُ
console.log(hasil.istilah[0]); // الماضي: وَعَدَ
console.log(hasil.istilah[1]); // المضارع: يَعِدُ - iilal: حذف فاء
console.log(hasil.istilah[2]); // الأمر: عِدْ - iilal: حذف + حذف همزة وصل
console.log(hasil.lughowi.mudhari[7]); // هم: يَعِدُوْنَ - iilal: حذف فاء
console.log(hasil.lughowi.isimMaful[0]); // مَعْدُوْدٌ - iilal: حذف فاء